...
module.exports = {
  async publicFind(ctx) {
    ...
    const data = await strapi.entityService.findMany("api::human-right.human-right", {
      ...query,
  ...
  async publicFindOne(ctx) {
    const { id } = ctx.params;
    const query = sanitizeQuery(ctx);
    const data = await strapi.entityService.findOne("api::human-right.human-right", id, {
      ...query,
      publicationState: "live",
    });
